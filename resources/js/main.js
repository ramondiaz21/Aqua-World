var error =
  "Ocurrió un error insesperado en el sitio, por favor intentelo mas tarde o pongase en contacto con su administrador.";

// =========== ALERTAS =============================================================================

// https://github.com/scottoffen/jquery.toaster
function toast2(tipo, titulo, msj, time) {
  //necesita el archivo jquery.toaster.js
  $.toaster({
    priority: tipo,
    title: titulo,
    message: msj,
...
// https://github.com/scottoffen/jquery.toaster
var cont = 0;
function toast(tipo, msj, time) {
  //necesita el archivo jquery.toaster.js
  $("html, body").animate({ scrollTop: 0 }, 800);

  if (cont != 0) $toaster.remove();
...
function validaCampoLength(idCampo, length) {
  ...
  $(document).on("keyup", "#" + idCampo, function (event) {
    ...
    if (caracteres.length > length) {
      toast1(
        "Error!",
        "No se pueden agregar mas de " + length + " caracteres",
        4000,
        "error"
      );
...
function validaCampoNum(idCampo, val) {
  ...
  $(document).on("keyup", "#" + idCampo, function (event) {
    ...
    if (caracteres > val) {
      toast1(
        "Error!",
        "No se puede ingresar un valor tan grande, intente con uno mas pequeño",
        4000,
        "error"
      );
...
function validarEmail(email) {
  ...
  if (patronArroba.test(email)) {
    //reviso que exista una @
...
function msgAlert(id, tipo, texto) {
  ...
  $("#" + id).html(
    '<button type="button" class="close" aria-label="Close" onclick="msgClose(\'' +
      id +
